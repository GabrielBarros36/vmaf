...
| `plans/optimization-opportunities.md` | 50+ catalogued inference-path optimisation opportunities (SIMD gaps, per-frame allocations, algorithmic redundancy, cache patterns, parallelism, Python overhead), each with file/line references, impact rating, and suggested fix. Read this before starting any performance work. Sections 1–12 have been addressed — see reports below. |
...
| `plans/sec7-simd-gaps-report.md` | Report on Section 7 additional SIMD gaps: motion y_convolution AVX2 (7a), VIF filter coefficient hoisting (7c), ADM adm_csf scale-0 AVX2 (7g), ADM adm_csf_den cubing AVX2 (7h), ADM angle_flag integer sign check (7e partial). Deferred: 7b, 7d, 7f, 7i. 67/67 tests pass, bit-identical scores, **3.1% speedup**. |
| `plans/sec8-memory-optimizations-report.md` | Report on Section 8 memory optimizations: float extractor buffer pre-allocation (8a — 17 malloc/free per frame → 0), AVX2 picture_copy for float conversion (8c). Skipped: 8b. 67/67 tests pass, bit-identical scores. |
| `plans/sec9-algorithmic-optimizations-report.md` | Report on Section 9 algorithmic optimizations: PSNR-HVS mask table precomputation (9a), CIEDE2000 pow() elimination (9b — cbrt, precomputed constants, repeated squaring). 67/67 tests pass, bit-identical scores. |
| `plans/sec10-simd-gaps-report.md` | Report on Section 10 secondary extractor SIMD gaps: PSNR-HVS AVX2 8×8 DCT (10a), CAMBI filter_mode AVX2 (10c), CAMBI mask block skip (10d), CAMBI decimate_shift AVX2 (10e). Deferred: 10b (FP order), 10f (too broad), 10g (too invasive). 67/67 tests pass, bit-identical scores. |
| `plans/sec11-python-orchestration-report.md` | Report on Section 11 Python orchestration: pre-compile regex (11a), cache loaded model (11c), pre-build feature key mapping (11d). Deferred: 11b (backward compat). 67/67 tests pass, bit-identical scores. |
| `plans/sec12-infrastructure-report.md` | Report on Section 12 infrastructure: framesync free-list hint pointer (12b), skip validation after first frame (12c). Deferred: 12a (thread-safety risk). 67/67 tests pass, bit-identical scores. |
