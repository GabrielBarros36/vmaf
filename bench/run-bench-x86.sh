...
# ---- Configuration -----------------------------------------------------------
INSTANCE_ID="i-025bf8165235858b4"
...
NUMA_NODE=0
THREADS=4                   # Match BENCH_CORES count
...
# CPU frequency governor: performance on all cores
for g in /sys/devices/system/cpu/cpu*/cpufreq/scaling_governor; do
    echo performance > "$g"
done

# AMD turbo boost: off (prevents frequency jitter between runs)
...
if [ ! -d libvmaf/build-prof ]; then
    .venv/bin/meson setup libvmaf/build-prof libvmaf \
        ...
        -Dc_args="-fno-omit-frame-pointer -O3"
fi
...
remote sudo perf stat \
    --repeat "$RUNS" \
    --event cycles,instructions,cache-references,cache-misses,branch-misses \
...
set -e
perf record -F $PERF_FREQ --call-graph=fp -o /tmp/vmaf-perf.data -- \\
    taskset -c "$BENCH_CORES" \\
    numactl --cpunodebind=$NUMA_NODE --membind=$NUMA_NODE \\
    "$PROF_BIN" \\
        --reference  "$REF_YUV" \\
...
# =============================================================================
{
    ...
    grep -E "seconds time elapsed|cycles|instructions|cache-references|cache-misses|branch-miss|insns per cycle" \
